        ...
        <div class="space-y-4">
          <Card>
            <CardContent class="p-6">
              <h3 class="font-semibold text-gray-900 mb-2">Puis-je utiliser autre chose qu'une carte bancaire ?</h3>
              <p class="text-gray-600">Non, pour simplifier et sécuriser les paiements, nous acceptons uniquement les cartes bancaires (Visa, Mastercard, American Express).</p>
            </CardContent>
          </Card>
...
const confirmPayment = async () => {
  ...
  try {
    ...
    if (error) {
      ...
      // Activer l'abonnement côté backend
      const userData = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('user') || '{}') : {}
      ...
      if (userId) {
        try {
          ...
          if (activationResponse.ok) {
            ...
            alert('Paiement réussi mais erreur d\'activation. Contactez le support.')
          }
        } catch (activationError) {
          console.error('Erreur d\'activation:', activationError)
          alert('Paiement réussi mais erreur d\'activation. Contactez le support.')
        }
      } else {
        alert('Paiement réussi ! Votre abonnement premium est maintenant actif.')
